...
/**
 * Get ranked list of agents for a task
 */
export function rankAgentsForTask(
  ...
  const graph = router.induceTopology(0, task, [taskDescriptor, ...agentDescriptors]);

  const rankings = agents.map((profile) => {
    const edge = graph.edges.find((e) => e.from === profile.agentId && e.to === "task");
    return {
      agentId: profile.agentId,
      score: edge?.similarity || 0,
      profile,
    };
  });
...
export function registerAgent(profile: AgentProfile): void {
  const existing = AGENT_REGISTRY.findIndex((a) => a.agentId === profile.agentId);
  if (existing >= 0) {
    AGENT_REGISTRY[existing] = profile;
  } else {
    AGENT_REGISTRY.push(profile);
  }
  log.info(`[DyTopo] Registered agent: ${profile.agentId}`);
}
