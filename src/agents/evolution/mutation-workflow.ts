...
export type MutationStatus = "success" | "failed" | "skipped" | "throttled";
...
export class MutationWorkflow {
  ...
  async run(): Promise<MutationResult[]> {
    ...
    for (const hotspot of prioritizedHotspots) {
      ...
      if (!canSpawn.allowed) {
        log.info(
          `[mutation] Throttled ${hotspot.toolName} - diagnostic already in progress: ${canSpawn.reason}`,
        );
        results.push({
          success: false,
          status: "throttled",
  ...
  private async processHotspot(hotspot: ToolHotspot): Promise<MutationResult> {
    try {
      ...
      if (!diagnostic.proposedFix) {
        log.info(`[mutation] Skipped ${hotspot.toolName} - no fix needed or already fixed.`);
        return { success: false, status: "skipped", error: "No fix proposed by diagnostic agent" };
      }
      ...
      if (!validation.allowed) {
        ...
        return { success: false, status: "failed", error: `Policy blocked: ${validation.reason}` };
      }
      ...
      if (!stats.success) {
        ...
        return {
          ...
          status: "failed",
          patchId,
          error: `Dojo verification failed: ${stats.error}`,
        };
      }
      ...
      if (!applyResult.success) {
        ...
        return {
          ...
          status: "failed",
          patchId,
          error: `Failed to apply: ${applyResult.error}`,
        };
      }
    ...
    try {
      ...
      if (failure.type === "build-failure") {
        ...
        return {
          success: false,
          status: "skipped",
          error: `Unhandled failure type: ${failure.type}`,
        };
      }
    } catch (err) {
      const error = err instanceof Error ? err.message : String(err);
