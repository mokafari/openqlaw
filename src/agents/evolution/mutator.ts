...
/**
 * Resolve the OpenClaw source repo root by walking up from the current module.
 * This is used for git operations which must run from the repo root, not the
 * agent workspace directory.
 */
function resolveRepoRoot(): string {
  // Start from the compiled JS file location (dist/agents/evolution/mutator.js)
  // and walk up to find the repo root (where .git exists)
  const currentFile = fileURLToPath(import.meta.url);
  let dir = dirname(currentFile);

  // Walk up looking for .git directory (max 10 levels to prevent infinite loop)
  for (let i = 0; i < 10; i++) {
  ...
  // Fallback to process.cwd() if we can't find .git
  log.warn("[mutator] Could not find repo root, falling back to process.cwd()");
  return process.cwd();
}
import {
  savePatch,
...
export class Mutator {
  ...
  async identifyFileHotspots(params?: {
    ...
    // Use repo root for git operations, not the agent workspace
  ...
  async commitAndNotify(patch: PatchMetadata): Promise<void> {
    ...
    try {
      ...
      // Use repo root for build and git operations
